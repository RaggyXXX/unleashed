...
#  Auto-detects Porofessor, Blitz & OP.GG, shows live status, installs
...
$Apps = @{
  ...
  opgg = @{ key='opgg'; name='OP.GG'; appRel='Programs\OP.GG\OP.GG.exe'; procName='OP.GG';
            task='OPGGNoAds'; daemonFile='opgg-noads-daemon.ps1'; flagsFile='opgg-flags.ps1'; launcherFile='opgg-launcher.vbs';
            port='9224'; autoArg=''; runKeyName='OPGG';
            defDir="$env:LOCALAPPDATA\UnleashedBypass\OPGG"; legacyDir='';
            shortcuts=@("$env:USERPROFILE\Desktop\OP.GG.lnk","$env:APPDATA\Microsoft\Windows\Start Menu\Programs\OP.GG.lnk") }
}
$Order = @('poro','blitz','opgg')
...
foreach (`$s in @($shJoined)) {
  if (Test-Path `$s) {
    ...
    if (`$l.TargetPath -ne `$wscript -or `$l.Arguments -notlike "*`$launcher*" -or `$l.Arguments -notlike "*--ub-relaunch*") {
      `$l.TargetPath = `$wscript; `$l.Arguments = "``"`$launcher``" --ub-relaunch"
...
"@
  [IO.File]::WriteAllText((Join-Path $dir $a.flagsFile), $content, (New-Object Text.UTF8Encoding($true)))
}
...
function Write-LauncherVbs($a, $dir) {
  ...
  $vbs = @(
    ...
    '  If LCase(a) = "--ub-relaunch" Then',
    '    relaunch = True',
    '  Else',
    '    extra = extra & " " & a',
    '  End If',
    'Next',
    "' The user always starts the app themselves. We force-launch it ONLY when the",
    "' daemon's Heal asks for a relaunch (--ub-relaunch) after an anti-tamper port",
    "' drop, and only if the exe exists. At logon (Run key / task pass autoArg, not",
    "' --ub-relaunch) this just starts the daemon and lets it attach.",
    ('If relaunch And Len(app) > 0 And fso.FileExists(app) Then sh.Run q & app & q & " --remote-debugging-port=' + $a.port + '" & extra, 1, False'),
    '',
...
function Setup-Elevated($a, $launcher) {
  # Registers the RunLevel=Highest logon task that starts the ad-block DAEMON at
  # logon (NOT the app - the user always launches the app themselves). The Highest
  # run level lets the daemon attach to / heal the app even after "run as admin".
  # If the CLI is not admin, this triggers exactly ONE UAC prompt via a short-lived
  # elevated helper; after that the daemon runs elevated at every logon, no more UAC.
  $wscript = Join-Path $env:SystemRoot 'System32\wscript.exe'
  $tpl = @'
$ErrorActionPreference='SilentlyContinue'
Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" | Where-Object { $_.CommandLine -like '*__DAEMON__*' } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force }
Start-Sleep 1
$act = New-ScheduledTaskAction -Execute '__WSCRIPT__' -Argument '"__LAUNCHER__" __AUTOARG__'
...
Register-ScheduledTask -TaskName '__TASK__' -Action $act -Trigger $trg -Principal $prn -Settings $set -Force | Out-Null
Start-Process '__WSCRIPT__' -ArgumentList '"__LAUNCHER__" __AUTOARG__'
Remove-Item -LiteralPath $PSCommandPath -Force -ErrorAction SilentlyContinue
'@
  $body = $tpl.Replace('__DAEMON__',$a.daemonFile).Replace('__WSCRIPT__',$wscript).Replace('__LAUNCHER__',$launcher).Replace('__AUTOARG__',$a.autoArg).Replace('__TASK__',$a.task)
  if (IsAdmin) { Invoke-Expression $body; return [bool](Get-ScheduledTask -TaskName $a.task -ErrorAction SilentlyContinue) }
  Unregister-ScheduledTask -TaskName $a.task -Confirm:$false -ErrorAction SilentlyContinue
  $tmp = Join-Path $env:TEMP ('unl-setup-' + $a.task + '.ps1')
  [IO.File]::WriteAllText($tmp, $body, (New-Object Text.UTF8Encoding($true)))
  try { Start-Process powershell -ArgumentList '-NoProfile','-ExecutionPolicy','Bypass','-WindowStyle','Hidden','-File',$tmp -Verb RunAs | Out-Null }
  catch { Remove-Item $tmp -Force -ErrorAction SilentlyContinue; return $false }
  $ok = $false
  for ($i = 0; $i -lt 30; $i++) {
    Start-Sleep 1
    if (Get-ScheduledTask -TaskName $a.task -ErrorAction SilentlyContinue) { $ok = $true; break }
  }
  if ($ok) { Remove-Item $tmp -Force -ErrorAction SilentlyContinue }
  return $ok
}
...
function Install-App($a, $appExe, $dir) {
  ...
  Write-Host '     setting up silent elevated auto-start (approve the one UAC prompt)...' -ForegroundColor Cyan
  if (Setup-Elevated $a $launcher) {
    Write-Host "     elevated ad-block daemon configured. It runs at every logon and attaches" -ForegroundColor Green
    Write-Host "     when YOU open $($a.name) - no auto-launch, no elevation prompt, no further UAC." -ForegroundColor Green
  } else {
    Write-Host '     elevation was declined - continuing without the elevated task.' -ForegroundColor Yellow
    Write-Host "     (ad-block still works; the 'Erhoehen' / elevated-game case won't auto-recover)." -ForegroundColor DarkGray
...
function Remove-ElevatedTask($a) {
  # Removes the RunLevel=Highest logon task. If not admin, a one-shot elevated helper (one UAC)
  # kills the (possibly elevated) daemon and unregisters the task, so a non-admin uninstall never
  # leaves an orphaned elevated daemon/task behind.
  if (-not (Get-ScheduledTask -TaskName $a.task -ErrorAction SilentlyContinue)) { return $true }
  if (IsAdmin) { Unregister-ScheduledTask -TaskName $a.task -Confirm:$false -ErrorAction SilentlyContinue; return $true }
  $tmp = Join-Path $env:TEMP ('unl-rm-' + $a.task + '.ps1')
  $body = "Get-CimInstance Win32_Process -Filter `"Name='powershell.exe'`" | Where-Object { `$_.CommandLine -like '*" + $a.daemonFile + "*' } | ForEach-Object { Stop-Process -Id `$_.ProcessId -Force -ErrorAction SilentlyContinue }`r`nUnregister-ScheduledTask -TaskName '" + $a.task + "' -Confirm:`$false -ErrorAction SilentlyContinue`r`nRemove-Item -LiteralPath `$PSCommandPath -Force -ErrorAction SilentlyContinue"
  [IO.File]::WriteAllText($tmp, $body, (New-Object Text.UTF8Encoding($false)))
  try { Start-Process powershell -ArgumentList '-NoProfile','-ExecutionPolicy','Bypass','-WindowStyle','Hidden','-File',$tmp -Verb RunAs | Out-Null }
  catch { Remove-Item $tmp -Force -ErrorAction SilentlyContinue; return $false }
  $gone = $false
  for ($i = 0; $i -lt 20; $i++) { Start-Sleep 1; if (-not (Get-ScheduledTask -TaskName $a.task -ErrorAction SilentlyContinue)) { $gone = $true; break } }
  if ($gone) { Remove-Item $tmp -Force -ErrorAction SilentlyContinue }
  return $gone
}
...
function Uninstall-App($a, $appExe) {
  ...
  if (Get-ScheduledTask -TaskName $a.task -ErrorAction SilentlyContinue) {
    if (Remove-ElevatedTask $a) { Write-Host "     elevated task '$($a.task)' removed." -ForegroundColor DarkGray }
    else { Write-Host "     could not remove the elevated task '$($a.task)' (elevation declined)." -ForegroundColor Yellow }
  }
  Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" | Where-Object { $_.CommandLine -like "*$($a.daemonFile)*" } | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }
  Start-Sleep 1
  $runKey = 'HKCU:\Software\Microsoft\Windows\CurrentVersion\Run'
...
function New-State {
  [pscustomobject]@{ enabled=$false; base=$UPDATE_BASE; cliPath=$env:SELF;
    apps=[pscustomobject]@{ poro=[pscustomobject]@{ dir=$Apps['poro'].defDir; daemon=$Apps['poro'].daemonFile }; blitz=[pscustomobject]@{ dir=$Apps['blitz'].defDir; daemon=$Apps['blitz'].daemonFile }; opgg=[pscustomobject]@{ dir=$Apps['opgg'].defDir; daemon=$Apps['opgg'].daemonFile } };
    versions=[pscustomobject]@{ cli=$VERSION; poro='0.0.0'; blitz='0.0.0'; opgg='0.0.0' } }
}
function Ensure-Versions($st) {
  if (-not ($st.PSObject.Properties['versions'])) { $st | Add-Member -NotePropertyName versions -NotePropertyValue ([pscustomobject]@{ cli=$VERSION; poro='0.0.0'; blitz='0.0.0'; opgg='0.0.0' }) -Force }
  return $st
}
...
function Get-Notify { $st = Load-State; if ($st -and $st.PSObject.Properties['notify'] -and $st.notify) { return [string]$st.notify }; return 'banner' }
function Set-Notify($v) { $st = Load-State; if (-not $st) { $st = New-State }; if ($st.PSObject.Properties['notify']) { $st.notify = $v } else { $st | Add-Member -NotePropertyName notify -NotePropertyValue $v -Force }; Save-State $st }
...
function Update-Self {
  ...
  if (-not (Get-WebFileBar $url $tmp 'installer')) { Write-Host '   Download failed.' -ForegroundColor Red; return }
...
# --- update UX: progress bars + startup checks ------------------------------
function Draw-Bar($label, $cur, $total) {
  $w = 26
  $pct = if ($total -gt 0) { [Math]::Min(1.0, [double]$cur / $total) } else { 0 }
  $fill = [int]($pct * $w)
  $bar = ('#' * $fill) + ('-' * ($w - $fill))
  Write-Host ("`r   {0,-15}[{1}] {2,3}%  {3,6:N0} KB" -f $label, $bar, [int]($pct*100), ($cur/1kb)) -NoNewline -ForegroundColor Cyan
}
function Get-WebFileBar($url, $path, $label) {
  $fs = $null; $st = $null; $resp = $null
  try {
    try { [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12 } catch {}
    $req = [Net.HttpWebRequest]::Create($url); $req.Timeout = 15000; $req.ReadWriteTimeout = 30000; $req.UserAgent = 'UnleashedBypass'
    $resp = $req.GetResponse(); $total = $resp.ContentLength; $st = $resp.GetResponseStream()
    $fs = [IO.File]::Create($path)
    $buf = New-Object byte[] 16384; $sofar = 0; $read = 0
    while (($read = $st.Read($buf, 0, $buf.Length)) -gt 0) { $fs.Write($buf, 0, $read); $sofar += $read; Draw-Bar $label $sofar $total }
    $fs.Close(); $st.Close(); $resp.Close()
    Draw-Bar $label $sofar ([Math]::Max($sofar, $total)); Write-Host ''
    return $true
  } catch {
...
function Update-Daemon($a) {
  $man = Get-RemoteManifest; if (-not $man) { return }
  $m = $man.($a.key); if (-not $m) { return }
  $s = Get-AppStatus $a
  $dir = if ($s.instDir -and (Test-Path $s.instDir)) { $s.instDir } else { $a.defDir }
  $target = Join-Path $dir $a.daemonFile
  $url = if ($m.PSObject.Properties['url'] -and $m.url) { [string]$m.url } else { (Get-StateBase) + '/' + $m.file }
  $tmp = $target + '.dl'
  if (-not (Get-WebFileBar $url $tmp ($a.name + ' daemon'))) { Write-Host '   download failed.' -ForegroundColor Red; return }
  if ((Get-Sha256 $tmp) -ne ([string]$m.sha256).ToLower()) { Remove-Item $tmp -Force -ErrorAction SilentlyContinue; Write-Host '   checksum mismatch.' -ForegroundColor Red; return }
  $procs = Get-CimInstance Win32_Process -Filter "Name='powershell.exe'" | Where-Object { $_.CommandLine -like ('*' + $a.daemonFile + '*') }
  try { Move-Item $tmp $target -Force } catch { Remove-Item $tmp -Force -ErrorAction SilentlyContinue; Write-Host '   write failed.' -ForegroundColor Red; return }
  Set-StateVersion $a.key ([string]$m.version)
  if ($procs) { $procs | ForEach-Object { Stop-Process -Id $_.ProcessId -Force -ErrorAction SilentlyContinue }; Start-Sleep 2; Start-Process 'powershell.exe' -WindowStyle Hidden -ArgumentList ('-NoProfile -ExecutionPolicy Bypass -File "' + $target + '"') }
  Write-Host ('   ' + $a.name + ' updated to v' + $m.version + '.') -ForegroundColor Green
}
function Invoke-StartupUpdates {
  ...
  if ($c) {
    Write-Host ''
    Write-Host ('   Installer update required:  v' + $VERSION + '  ->  v' + $c.version) -ForegroundColor Yellow
    Update-Self
  }
  foreach ($k in $Order) {
    $a = $Apps[$k]
    $s = Get-AppStatus $a
    if (-not $s.installed) { continue }
    $vi = Get-CompVer $k
    if ($vi.upd) {
      Write-Host ''
      Write-Host ('   ' + $a.name + ' daemon update:  v' + $vi.inst + '  ->  v' + $vi.lat) -ForegroundColor Yellow
      $ans = Read-Host '   Update now? [Y/n]'
      if ($ans -notmatch '^[nN]') { Update-Daemon $a }
    }
  }
}
...
$taskOf = @{ poro='PorofessorNoAds'; blitz='BlitzNoAds'; opgg='OPGGNoAds' }
foreach($k in @('poro','blitz','opgg')){
  ...
  if(Test-Path $dp){
    ...
    if(ApplyComp $k $dp){
      $changed=$true
      try { if($taskOf[$k]){ Start-ScheduledTask -TaskName $taskOf[$k] -ErrorAction SilentlyContinue } } catch {}
      if($procs){
...
function Install-Updater($base) {
  ...
  if (-not ($st.PSObject.Properties['apps'])) { $st | Add-Member -NotePropertyName apps -NotePropertyValue ([pscustomobject]@{ poro=[pscustomobject]@{ dir=$Apps['poro'].defDir; daemon=$Apps['poro'].daemonFile }; blitz=[pscustomobject]@{ dir=$Apps['blitz'].defDir; daemon=$Apps['blitz'].daemonFile }; opgg=[pscustomobject]@{ dir=$Apps['opgg'].defDir; daemon=$Apps['opgg'].daemonFile } }) -Force }
  if ($st.PSObject.Properties['notify']) { $st.notify = 'banner' } else { $st | Add-Member -NotePropertyName notify -NotePropertyValue 'banner' -Force }
  Save-State $st
...
function Remove-Updater {
  if (Test-Path $UpdaterCfg) {
    try { $c = [IO.File]::ReadAllText($UpdaterCfg) | ConvertFrom-Json; $c.enabled=$false; if ($c.PSObject.Properties['notify']) { $c.notify='off' } else { $c | Add-Member -NotePropertyName notify -NotePropertyValue 'off' -Force }; [IO.File]::WriteAllText($UpdaterCfg, ($c | ConvertTo-Json -Depth 6), (New-Object Text.UTF8Encoding($false))) } catch {}
  }
  ...
  foreach($f in @($UpdaterScript,$UpdaterVbs)){ Remove-Item $f -Force -ErrorAction SilentlyContinue }
}
...
function Menu-Updater {
  while ($true) {
    ...
    foreach ($comp in @('cli','poro','blitz','opgg')) {
      $vi = Get-CompVer $comp
    ...
    switch ($uc) {
      ...
      '3' { Invoke-StartupUpdates; Pause2 }
      ...
      '6' { $n = Get-Notify; $nx = if ($n -eq 'banner') { 'auto' } elseif ($n -eq 'auto') { 'off' } else { 'banner' }; Set-Notify $nx; Write-Host ('   In-app banner mode set to: ' + $nx) -ForegroundColor Green; Pause2 }
      '0' { return }
      default { }
    }
  }
...
function Pick-App($verb) {
  Write-Host ''
  $w = Read-Host "   $verb which product?  [1] Porofessor   [2] Blitz   [3] OP.GG"
  if ($w -eq '1') { return $Apps['poro'] } elseif ($w -eq '2') { return $Apps['blitz'] } elseif ($w -eq '3') { return $Apps['opgg'] }
  return $null
}
...
while ($true) {
  ...
  Aopt '2' 'Install single product' $null
  Grp 'maintain'
  Aopt 'R' 'Repair / re-point all installs' $null
  Aopt '3' 'Uninstall' $null
  Grp 'system'
  ...
  switch ($c) {
    ...
    '2' {
      $a = Pick-App 'Install'
      if ($a) { Install-App $a $null $null; Offer-Updater } else { Write-Host '   Invalid choice.' -ForegroundColor Red }
      Pause2
    }
    'r' {
      Write-Host ''
    ...
    '3' {
      $a = Pick-App 'Uninstall'
      if ($a) { Uninstall-App $a $null } else { Write-Host '   Invalid choice.' -ForegroundColor Red }
      Pause2
    }
    '7' {
      Write-Host ''
      $w = Read-Host '   Which app?  [1] Porofessor   [2] Blitz   [3] OP.GG'
      $a = $null; if ($w -eq '1') { $a = $Apps['poro'] } elseif ($w -eq '2') { $a = $Apps['blitz'] } elseif ($w -eq '3') { $a = $Apps['opgg'] }
